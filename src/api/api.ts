...
export const api = {
  async getNotes(userId: string): Promise<Note[]> {
    if (!navigator.onLine) return getLocalCache().filter(n => !n.isDeleted && n.userId === userId);
    try {
      const res = await fetch(`${API_URL}?userId=${userId}`);
      if (!res.ok) throw new Error('Network response was not ok');
      const data = await res.json();

      // Update local cache with fetched notes
      const cache = getLocalCache().filter(n => n.userId !== userId);
      saveLocalCache([...cache, ...data]);

      return data.filter((n: Note) => !n.isDeleted);
    } catch (error) {
      console.warn('API fetch failed, falling back to cache', error);
      return getLocalCache().filter(n => !n.isDeleted && n.userId === userId);
    }
  },

  async createNote(data: Partial<Note>, userId: string): Promise<Note> {
    const newNote = { ...data, id: uuidv4(), userId, createdAt: Date.now(), updatedAt: Date.now() } as Note;
    ...
    try {
      ...
      saveLocalCache([created, ...getLocalCache().filter(n => n.id !== created.id)]);
      return created;
    } catch (error) {
      this.queueAction('POST', newNote.id, newNote);
      saveLocalCache([newNote, ...getLocalCache()]);

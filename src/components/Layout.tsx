...
import { WifiOff, Wifi, Github, Linkedin, Instagram, Command, Menu, LayoutDashboard, Archive, LogOut, RefreshCw, FileText, Sparkles } from 'lucide-react';
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full py-6 px-4">
      ...
      <div className="mt-auto pt-6 border-t border-slate-200 dark:border-slate-800 space-y-2">
        ...
        <button onClick={() => setIsCmdOpen(true)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800/50 transition-colors">
        ...
        <footer className="border-t border-slate-200 dark:border-slate-800 py-8 mt-auto bg-white dark:bg-slate-900">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex flex-col items-center md:items-start gap-1">
              <p className="text-sm text-slate-500">Built by <span className="font-semibold text-slate-900 dark:text-white">VS Creation</span></p>
              <p className="text-xs text-slate-400">Frontend Developer</p>
            </div>
            <div className="flex items-center gap-6">
              <a href="https://github.com/yourname" target="_blank" rel="noreferrer" className="text-slate-400 hover:text-indigo-600 dark:hover:text-violet-400 transition-colors flex items-center gap-2 text-sm font-medium"><Github className="w-4 h-4" /> <span className="hidden sm:inline">GitHub</span></a>
              <a href="https://linkedin.com" target="_blank" rel="noreferrer" className="text-slate-400 hover:text-indigo-600 dark:hover:text-violet-400 transition-colors flex items-center gap-2 text-sm font-medium"><Linkedin className="w-4 h-4" /> <span className="hidden sm:inline">LinkedIn</span></a>

...
import { Archive, Command, FileText, Github, Instagram, LayoutDashboard, Linkedin, Menu, RefreshCw, Settings, Wifi, WifiOff } from 'lucide-react';
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full py-6 px-4">
      ...
      <div className="mt-auto pt-6 border-t border-slate-200 dark:border-slate-800 space-y-2">
        {/* User Profile Mock */}
        <div className="flex items-center gap-3 px-3 py-2 mb-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800/50 cursor-pointer transition-colors">
          <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-500 flex items-center justify-center text-white font-bold text-xs">CC</div>
          <div className="flex-1 overflow-hidden">
            <p className="text-sm font-medium text-slate-900 dark:text-white truncate">ConceptualCode</p>
            <p className="text-xs text-slate-500 truncate">Pro Plan</p>
          </div>
          <Settings className="w-4 h-4 text-slate-400" />
        </div>

        <button onClick={() => setIsCmdOpen(true)} className="w-full flex items-center gap-3 px-0 py-2.5 rounded-xl text-sm font-medium text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800/50 transition-colors">
        ...
        {isSidebarOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed inset-y-0 left-0 z-50 w-72 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 md:hidden"
            >
              <SidebarContent />
            </motion.aside>
          </>
        ...
        <header className="sticky top-0 z-30 glass">
          <div className="px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 sm:gap-4 ml-auto">
              <div
                className={`flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full border ${isOffline ? "bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-900/20 dark:border-amber-800/50" : isSyncing ? "bg-blue-50 text-blue-600 border-blue-200 dark:bg-blue-900/20 dark:border-blue-800/50" : "bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-900/20 dark:border-emerald-800/50"}`}
              >
                {isOffline ? (
        ...
        <footer className="border-t border-slate-200 dark:border-slate-800 py-8 mt-auto bg-white dark:bg-slate-900">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex flex-col items-center md:items-start gap-1">
              <p className="text-sm text-slate-500">
                ...
                <a
                  ...
                  href="https://conceptualcode.rf.gd/en"
                >
                  Conceptualcode
                </a>
              </p>
            ...
            <div className="flex items-center gap-6">
              <a
                href="https://github.com/conceptualcode-official"
                target="_blank"
                rel="noreferrer"
                className="text-slate-400 hover:text-indigo-600 dark:hover:text-violet-400 transition-colors flex items-center gap-2 text-sm font-medium"
              >
                <Github className="w-4 h-4" />{" "}
                <span className="hidden sm:inline">GitHub</span>
              </a>
              <a
                href="https://www.linkedin.com/in/vivek-kumar786/"
                target="_blank"
                rel="noreferrer"
                className="text-slate-400 hover:text-indigo-600 dark:hover:text-violet-400 transition-colors flex items-center gap-2 text-sm font-medium"
              >
              ...
              <a
                href="https://instagram.com/conceptualcode"
                target="_blank"
                rel="noreferrer"
                className="text-slate-400 hover:text-indigo-600 dark:hover:text-violet-400 transition-colors flex items-center gap-2 text-sm font-medium"
              >

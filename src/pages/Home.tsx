...
import { Search, Plus, SlidersHorizontal, FileText, LayoutGrid, Archive, Activity, Clock, Pin, Hash } from 'lucide-react';
      ...
      <div className="space-y-8 animate-fade-in pb-24">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col gap-2">
            <div className="text-slate-500 dark:text-slate-400 text-sm font-medium flex items-center gap-2"><LayoutGrid className="w-4 h-4"/> Total Notes</div>
            <div className="text-3xl font-bold text-slate-900 dark:text-white">{notes.length}</div>
          </div>
          <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col gap-2">
            <div className="text-slate-500 dark:text-slate-400 text-sm font-medium flex items-center gap-2"><Pin className="w-4 h-4 text-indigo-500"/> Pinned</div>
            <div className="text-3xl font-bold text-slate-900 dark:text-white">{dashPinned.length}</div>
          </div>
          <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col gap-2">
            <div className="text-slate-500 dark:text-slate-400 text-sm font-medium flex items-center gap-2"><Archive className="w-4 h-4 text-slate-500"/> Archived</div>
            <div className="text-3xl font-bold text-slate-900 dark:text-white">{notes.filter(n=>n.isArchived).length}</div>
          </div>
          <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col gap-2">
            <div className="text-slate-500 dark:text-slate-400 text-sm font-medium flex items-center gap-2"><Activity className="w-4 h-4 text-emerald-500"/> Tags</div>
            <div className="text-3xl font-bold text-slate-900 dark:text-white">{allTags.length}</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2"><Clock className="w-5 h-5 text-indigo-500"/> Recently Edited</h2>
            </div>
          ...
          <div className="space-y-6">
            <h2 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2"><Pin className="w-5 h-5 text-indigo-500"/> Pinned Notes</h2>
            <div className="flex flex-col gap-4">
              {dashPinned.length === 0 ? (
                <div className="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700 border-dashed text-center text-slate-500 text-sm">No pinned notes yet.</div>
              ) : (
                dashPinned.slice(0, 3).map(note => (
                  <div key={note.id} onClick={() => setFocusNote(note)} className="p-4 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md cursor-pointer transition-all">
                    <h4 className="font-semibold text-slate-900 dark:text-white line-clamp-1 mb-1">{note.title || 'Untitled'}</h4>
